...
namespace OED_Console2026.Classes.SpectreSamples;
...
internal class DisplaySamples
{
    ...
    /// <summary>
    /// Prompts the user to input their first name, last name, and birthday using Spectre.Console.
    /// </summary>
    /// <remarks>
    /// This method uses <see cref="Spectre.Console.AnsiConsole.Ask{T}(string)"/> to gather user input
    /// for the first name, last name, and birthday. The prompts are styled with Spectre.Console's 
    /// formatting capabilities to enhance user interaction.
    /// </remarks>
    /// <returns>
    /// A <see cref="Classes.System.Models.Human"/> object containing the user's 
    /// first name, last name, and birthday.
    /// </returns>
    public static Human GetUserDetails()
    {
        Human user = new()
        {
            FirstName = AnsiConsole.Ask<string>("What's your [cyan]first name[/]?"),
            LastName = AnsiConsole.Ask<string>("What's your [cyan]last name[/]?"),
            Birthday = AnsiConsole.Ask<DateOnly>("What's [cyan]your birthday?[/] ([Gray]MM/DD/YYYY[/]) ")
        };
